...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: lizus.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: lizusd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizelizusamount=<n> " + _("Keep N Lizus anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Lizus is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Lizus is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Lizus version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Lizus server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Lizus") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Lizus to complete") << "\n";
    ...
    nAnonymizeLizusAmount = GetArg("-anonymizelizusamount", 0);
    if(nAnonymizeLizusAmount > 999999) nAnonymizeLizusAmount = 999999;
    if(nAnonymizeLizusAmount < 2) nAnonymizeLizusAmount = 2;
    ...
    LogPrintf("Anonymize Lizus Amount %d\n", nAnonymizeLizusAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1LIZ+1000 == (.1LIZ+100)*10
       10LIZ+10000 == (1LIZ+1000)*10
    */
