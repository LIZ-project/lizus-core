...
#include "lizusnodeconfigdialog.h"
...
void MasternodeManager::updateLizusNode(QString alias, QString addr, QString privkey, QString txHash, QString txIndex, QString rewardAddress, QString rewardPercentage, QString status)
{
    LOCK(cs_lizus);
    bool bFound = false;
    int nodeRow = 0;
...
{
    AddEditLizusNode* aenode = new AddEditLizusNode();
    aenode->exec();
}
...
void MasternodeManager::on_UpdateButton_clicked()
{
    BOOST_FOREACH(CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
        ...
        std::vector<CMasternode> vMasternodes = mnodeman.GetFullMasternodeVector();
        if (errorMessage == ""){
            updateLizusNode(QString::fromStdString(mne.getAlias()), QString::fromStdString(mne.getIp()), QString::fromStdString(mne.getPrivKey()), QString::fromStdString(mne.getTxHash()),
