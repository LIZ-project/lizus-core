...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeLizusAmount"))
        settings.setValue("nAnonymizeLizusAmount", 1000);
    nAnonymizeLizusAmount = settings.value("nAnonymizeLizusAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeLizusAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeLizusAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeLizusAmount:
            return QVariant(nAnonymizeLizusAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeLizusAmount:
            nAnonymizeLizusAmount = value.toInt();
            settings.setValue("nAnonymizeLizusAmount", nAnonymizeLizusAmount);
            emit AnonymizeLizusAmountChanged(nAnonymizeLizusAmount);
            break;
        default:
            break;
        }
