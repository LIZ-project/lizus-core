...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeLizusAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeLizusAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeLizusAmount = strAnonymizeLizusAmount.remove(strAnonymizeLizusAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLizusAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeLizusAmount*COIN) nMaxToAnonymize = nAnonymizeLizusAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeLizusAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeLizusAmount));
        strAnonymizeLizusAmount = strAnonymizeLizusAmount.remove(strAnonymizeLizusAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLizusAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
